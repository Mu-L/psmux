...
/// Build an AppState with one window containing a real (10×5) pane that has
/// some text in the visible rows so we can verify range capture.
fn make_app_with_pane_content() -> AppState {
    let mut app = AppState::new("capture_test".to_string());
...
// ════════════════════════════════════════════════════════════════════════════
// capture-pane -S/-E range semantics
// ════════════════════════════════════════════════════════════════════════════

/// The old code treated negative S as relative to the BOTTOM of the visible
/// area: start = (last_row + v + 1).  A 50-row pane with -S=-5 gave start=45,
/// i.e. rows 45-49.  In a fresh session those rows are blank so before==after.
///
/// The new code clamps negative S to 0, matching tmux (where negative = scrollback
/// history; with no history the start saturates to row 0).
#[test]
fn negative_s_clamps_to_zero_not_bottom() {
    // Verify the formula: NEW behaviour should give start=0 for any negative S.
    // Old formula: start = (49 + (-5) + 1) = 45
    // New formula: start = 0
    let last_row: i32 = 49; // 50-row pane, 0-indexed
    let s: i32 = -5;

    // Old (buggy) computation
    let old_start = (last_row + s + 1).max(0) as u16;
    assert_eq!(old_start, 45, "old formula should give 45 (rows 45-49 = empty)");

    // New (correct) computation
    let new_start: u16 = 0; // negative clamps to 0
    assert_ne!(old_start, new_start, "fix must change the start row");
    assert_eq!(new_start, 0, "negative S must clamp to row 0");
}

#[test]
fn positive_s_still_absolute() {
    // Positive S values remain unchanged: start = S (clamped to last_row).
    let last_row: u16 = 49;
    for s in [0i32, 5, 10, 49] {
        let start = (s as u16).min(last_row);
        assert_eq!(start, s as u16, "positive S={s} must be absolute");
    }
}
...
fn positive_s_clamped_to_last_row() {
    let last_row: u16 = 49;
    let s: i32 = 100; // beyond pane height
    let start = (s as u16).min(last_row);
    assert_eq!(start, last_row, "S beyond pane height must clamp to last row");
}

#[test]
fn negative_e_clamps_to_zero() {
    // -E behaves the same way: negative = scrollback history offset,
    // clamp to 0 when no history is available.
    let e: i32 = -3;
    let end: u16 = if e < 0 { 0 } else { e as u16 };
    assert_eq!(end, 0, "negative E must clamp to row 0");
}

#[test]
fn default_end_is_last_row() {
    // When -E is omitted the default must be the last visible row, not 0.
    let last_row: u16 = 49;
    let end: u16 = last_row; // None => last_row
    assert_eq!(end, 49);
}

#[test]
fn s_minus_5_returns_full_screen_for_50_row_pane() {
    // The key invariant: -S -5 on a 50-row pane must include row 0, so that
    // an idle PS prompt at row 5 is captured and changes are detected.
    let new_start: u16 = 0;   // after fix
    let end: u16 = 49;        // last row (default -E)
    let rows_captured = (end - new_start + 1) as usize;
    assert_eq!(rows_captured, 50, "capture -S -5 must return all 50 visible rows");
    assert!(new_start == 0, "start must include top of screen where PS prompt lives");
}

// ════════════════════════════════════════════════════════════════════════════
// NudgeSession content-change detection invariant
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// new-session -x/-y dimensions forwarding
// ════════════════════════════════════════════════════════════════════════════
